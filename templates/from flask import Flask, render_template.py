...
# =============================
# Route for canceling a leave request with a specific index (from URL)
@app.route('/cancel_leave/<int:index>', methods=['POST'])
def cancel_leave(index):

    # Get the currently logged-in employee ID from session
    emp_id = session.get('user_id')

    # Check if the index is valid (within the range of leave requests list)
    if not (0 <= index < len(LeaveSystem.leave_requests)):
        flash("Invalid request.", "danger")  # Show error message
        return redirect(url_for('employee_dashboard'))  # Redirect back to dashboard

    # Get the specific leave request based on index
    req = LeaveSystem.leave_requests[index]

    # SECURITY CHECK: Ensure the logged-in employee owns this leave request
    if str(req.get('id')) != str(emp_id):
        flash("Unauthorized action.", "danger")  # Show unauthorized message
        return redirect(url_for('employee_dashboard'))  # Redirect back

    # Get cancellation reason from the submitted form
    reason = request.form.get('cancel_reason')

    # Validate if reason is empty or only spaces
    if not reason or reason.strip() == "":
        flash("Error: Please provide a reason for cancellation.", "warning")
        return redirect(url_for('employee_dashboard'))  # Stop and go back

    # Process the cancellation request in the system
    if LeaveSystem.request_cancellation(index, reason):
        flash("Cancellation request sent to HR.", "success")  # Success message
    else:
        flash("Error: Cannot cancel leave or invalid request.", "danger")  # Failure message

    # Redirect back to employee dashboard after processing
    return redirect(url_for('employee_dashboard'))
